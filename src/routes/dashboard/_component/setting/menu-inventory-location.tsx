...
export default function MenuInventoryLocation() {
  ...
  // detail data
  const detailQuery = useDetailLocation({
    id: selectedId,
    enabled: !!selectedId,
  })
  // all data
  const allQuery = useLocation({
              ...
              >
                <div className='flex gap-4 items-center'>
                  <p
                    className={cn(
                      'text-dark',
                      selectedId === item.id && isDanger && 'text-red-primary text-sm',
                    )}
                  >
                    {item.name}
                    {selectedId === item.id &&
