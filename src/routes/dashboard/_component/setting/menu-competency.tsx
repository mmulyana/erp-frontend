...
export default function MenuCompetency() {
  const [open, setOpen] = useState(false)
  const [isDanger, setDanger] = useState(false)
  const [search, setSearch] = useState('')
  const [selectedId, setSelectedId] = useState<number | null>(null)

  const { mutate: create } = useCreateCompetency()
  const { mutate: update } = useUpdateCompetency()
  const { mutate: remove } = useDeleteCompetency()

  // detail data
  const detailQuery = useDetailCompetency({
    id: selectedId,
    enabled: !!selectedId,
  })
  // all data
  const allQuery = useCompetency({
      ...
      {open ? (
        ...
        <>
          <p className='text-sm text-dark/50'>
            Lihat semua kompetensi yang terdaftar di sistem
          </p>
