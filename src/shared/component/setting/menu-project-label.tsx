...
export default function MenuProjectLabel() {
  const [open, setOpen] = useState(false)
  const [isDanger, setDanger] = useState(false)
  const [search, setSearch] = useState('')
  const [selectedId, setSelectedId] = useState<number | null>(null)

  const { mutate: create } = useCreateProjectLabel()
  const { mutate: update } = useUpdateProjectLabel()
  const { mutate: remove } = useDeleteProjectLabel()

  // detail data
  const detailQuery = useDetailProjectLabel({
    id: selectedId,
    enabled: !!selectedId,
  })
  // all data
  const allQuery = useProjectLabels({
      ...
      {open ? (
        ...
        <>
          <p className='text-sm text-dark/50'>
            Lihat semua label untuk proyek yang terdaftar di sistem
          </p>
          <div className='mt-8 flex justify-between items-center'>
            <Search withoutUrl debounceTime={500} onSearch={setSearch} />
            <ProtectedComponent required={['label:create']}>
            ...
            <div className='flex flex-col gap-2'>
              {competencies?.map((item) => (
                ...
                >
                  ...
                  <ProtectedComponent
                    required={['label:delete', 'label:update']}
                  >
                    {(!isDanger || selectedId !== item.id) && (
                      <div className='absolute top-1/2 -right-[72px] -translate-y-1/2 group-hover:right-0 h-full w-fit px-3 flex justify-between items-center gap-4 duration-150 ease-in'>
                        <ProtectedComponent required={['label:delete']}>
                          <button
                            onClick={() => {
                        ...
                        <ProtectedComponent required={['label:update']}>
                          <button
                            onClick={() => {
                              setSelectedId(item.id)
                              setOpen(true)
