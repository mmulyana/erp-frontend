...
import {
  ...
  useDetailMeasurement,
  useMeasurement,
  useUpdateMeasurement,
} from '@/hooks/api/use-measurement'
...
export default function MenuInventoryMeasurement() {
  const [open, setOpen] = useState(false)
  const [isDanger, setDanger] = useState(false)
  const [search, setSearch] = useState('')
  const [selectedId, setSelectedId] = useState<number | null>(null)

  const { mutate: create } = useCreateMeasurement()
  const { mutate: update } = useUpdateMeasurement()
  const { mutate: remove } = useDeleteMeasurement()

  // detail data
  const detailQuery = useDetailMeasurement({
    id: selectedId,
    enabled: !!selectedId,
  })
  // all data
  const allQuery = useMeasurement({
